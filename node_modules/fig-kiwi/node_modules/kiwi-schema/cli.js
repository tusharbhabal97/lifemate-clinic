...
const cpp_callback_1 = require("./cpp-callback");
...
function main(args) {
    ...
    try {
        var parsed = typeof content === 'string' ? parser_1.parseSchema(content) : binary_1.decodeBinarySchema(new bb_1.ByteBuffer(content));
        var compiled = js_1.compileSchema(parsed);
    }
    catch (e) {
    ...
    if (flags['--js'] !== null) {
        writeFileString(flags['--js'], js_1.compileSchemaJS(parsed));
    }
    // Generate JavaScript code
    if (flags['--ts'] !== null) {
        writeFileString(flags['--ts'], ts_1.compileSchemaTypeScript(parsed));
    }
    // Generate C++ code
    if (flags['--cpp'] !== null) {
        writeFileString(flags['--cpp'], cpp_1.compileSchemaCPP(parsed));
    }
    if (flags['--callback-cpp'] !== null) {
        writeFileString(flags['--callback-cpp'], cpp_callback_1.compileSchemaCallbackCPP(parsed));
    }
    // Generate Skew code
    if (flags['--skew'] !== null) {
        writeFileString(flags['--skew'], skew_1.compileSchemaSkew(parsed));
    }
    ...
    if (flags['--binary'] !== null) {
        writeFileBuffer(flags['--binary'], Buffer.from(binary_1.encodeBinarySchema(parsed)));
    }
    // Generate a textual schema file
    if (flags['--text'] !== null) {
        writeFileBuffer(flags['--text'], Buffer.from(printer_1.prettyPrintSchema(parsed)));
    }
    ...
    if (flags['--to-json'] !== null) {
        ...
        writeFileString(flags['--to-json'] + '.json', JSON.stringify(compiled['decode' + rootType](new Uint8Array(fs.readFileSync(flags['--to-json']))), null, 2) + '\n');
    }
    ...
    if (flags['--from-json'] !== null) {
        ...
        writeFileBuffer(flags['--from-json'] + '.bin', Buffer.from(compiled['encode' + rootType](JSON.parse(fs.readFileSync(flags['--from-json'], 'utf8')))));
    }
    return 0;
}
exports.main = main;
